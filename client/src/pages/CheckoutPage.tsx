...
export const CheckoutPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || err.response?.data?.errors?.[0]?.message || 'Error al crear la reserva. Intenta de nuevo.';
      setError(msg);
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center pt-20 bg-surface-dim dark:bg-surface-950">
        ...
        >
          ...
          >
            <Check size={28} className="text-white md:hidden" />
            <Check size={36} className="text-white hidden md:block" />
          </motion.div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3 md:gap-4 mb-6 md:mb-8">
          <div className="bg-primary-500 rounded-xl md:rounded-2xl p-2 md:p-3 shadow-sm">
            ...
            <ShoppingCart size={24} className="text-white hidden md:block" />
          </div>
          <div>
            <span className="text-[0.6rem] md:text-[0.7rem] uppercase tracking-[0.08em] font-bold text-primary-500 dark:text-primary-400">
              Reserva
            </span>
            <h1 className="text-2xl md:text-3xl lg:text-4xl font-display font-bold text-ink dark:text-surface-100">
              Confirmar Reserva
            </h1>
          </div>
        </div>
        ...
        >
          <motion.div variants={itemVariants(reduce)} className="lg:col-span-3 space-y-4 md:space-y-6">
            <form onSubmit={handleSubmit}>
              <div className="bg-surface dark:bg-surface-800 rounded-xl md:rounded-2xl border border-surface-200 dark:border-surface-700 p-4 md:p-6 shadow-sm mb-4 md:mb-6">
                <h2 className="text-base md:text-lg font-display font-bold text-ink dark:text-surface-100 mb-4 md:mb-5">
                  Tus Datos
                </h2>
                <div className="space-y-3 md:space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      type="text"
                      value={customerName}
                      onChange={(e) => { setCustomerName(e.target.value); setValidation(prev => ({ ...prev, customerName: '' })); }}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => { setCustomerIdCard(e.target.value); setValidation(prev => ({ ...prev, customerIdCard: '' })); }}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      type="tel"
                      value={customerPhone}
                      onChange={(e) => { setCustomerPhone(e.target.value); setValidation(prev => ({ ...prev, customerPhone: '' })); }}
              ...
              <div className="bg-surface dark:bg-surface-800 rounded-xl md:rounded-2xl border border-surface-200 dark:border-surface-700 p-4 md:p-6 shadow-sm mb-4 md:mb-6">
                <h2 className="text-base md:text-lg font-display font-bold text-ink dark:text-surface-100 mb-3 md:mb-4">
                  Método de Pago
                </h2>
              ...
              >
                {loading ? (
                  ...
                  <>
                    Reservar Productos — {formatPrice(total)}
                    <ArrowRight size={18} />
                  </>
                )}
              </motion.button>
          ...
          <motion.div variants={itemVariants(reduce)} className="lg:col-span-2">
              ...
              <div className="space-y-2 md:space-y-3 max-h-48 md:max-h-64 overflow-y-auto mb-4 md:mb-5 pr-1">
                {items.map((item) => (
                  <div key={item.id} className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-semibold text-ink dark:text-surface-100 whitespace-nowrap">
